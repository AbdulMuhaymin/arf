...
class calculate_E_formation:
            ...
            for line in data_file:
                if "&VBM" in line:
                    ...
                    for i in range(self.charge_start, self.charge_end+1):
                        ...
                        self.correction_terms_list.append(float(tmp_line.split()[1])) #Eiso-Eperiodic
                else:
                    continue
    ...
    def write_data(self):
        ...
        for i in range(0, self.n_charges):
            ...
            if q == self.correction_terms[i,0]: # Should contain the charge state, q.
                E_correction = self.correction_terms[i,2] -1 *q *(-1) *self.correction_terms[i,1]
    ...
    def plot_data(self):
        ...
        for i in range(0, self.n_charges):
            ...
            label_str = repr('charge state = ' + str(q))
...
class sxdefectalign_plot:
    ...
    def __init__(self, filein, title):
        self.filein =filein #it should be 'vline-eV.dat'
        self.title = title # write the charge state only with plus/minus sign, used in plot title
        self.separate_vline()
        self.plot_vline()
    ...
    def plot_vline(self):
        ...
        axes.legend(loc=0, fontsize=15) #loc=1 - upper right / loc=2 - upper left / loc=3 - lower left / loc=4 - lower right
        axes.set_xlabel('z (Bohr)', fontsize = 20)
        axes.set_ylabel('Potential (eV)', fontsize = 20)
        plt.xticks(fontsize=15)
        plt.yticks(fontsize=15)
        axes.set_title('Potential Alignment, Co in ZnS, q=' + str(self.title) + " wrt pristine", fontsize = 22)
        #plt.ylim(-0.1,0.1)
        plt.tight_layout()
        fig.savefig('Co_q' + str(self.title).replace('+', 'p').replace('-', 'n') + '_wrt_pristine.png')
        plt.show()