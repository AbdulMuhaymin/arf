...
class sxdefectalign_plot:
    ...
    def plot_vline(self):
        ...
        axes.set_title('Potential Alignment, Co in ZnS, q=' + str(self.title) + " wrt q0", fontsize = 22)
        #plt.ylim(-0.1,0.1)

        fig.savefig('Co_q' + str(self.title).replace('+', 'p').replace('-', 'n') + '_wrt_q0_aligned.png')
        plt.show()